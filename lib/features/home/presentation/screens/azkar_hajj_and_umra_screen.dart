...
@RoutePage()
class AzkarHajjAndUmraScreen extends StatelessWidget {
  const AzkarHajjAndUmraScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: Nafa7atAppBar(title: context.localizedText.azkarHajjAndUmra),
      ),
      body: BlocBuilder<HomeCubit, HomeState>(
        builder: (context, state) {
          return ListView.separated(
            itemCount: state.azkarList.hajjAndUmrahAzkar.length,
            separatorBuilder: (context, index) => verticalSpace(20),
            itemBuilder: (context, index) {
              final item = state.azkarList.hajjAndUmrahAzkar;

...
@RoutePage()
class AzkarWuduScreen extends StatelessWidget {
  const AzkarWuduScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: Nafa7atAppBar(title: context.localizedText.azkarWudu),
      ),
      body: BlocBuilder<HomeCubit, HomeState>(
        builder: (context, state) {
          return ListView.separated(
            itemCount: state.azkarList.wuduAzkar.length,
            separatorBuilder: (context, index) => verticalSpace(20),
            itemBuilder: (context, index) {
              final item = state.azkarList.wuduAzkar;
